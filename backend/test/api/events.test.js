...
describe('controllers/events', async () => {
    ...
    it('add() dates is crossed ', async () => {
        const eventData = generateValidEventData();
        eventData['date_start'] = new Date('2021 10:40').toISOString();
        eventData['date_end'] = new Date('2021 10:50').toISOString();
        ...
        const event1 = await agent.post('/api/v1/events/add')
            ...
            .expect('Content-Type', /json/)
            .expect(201);
        console.log(JSON.stringify(event1.body));
        console.log('event2222222222222222222222222222222222222222222');
        console.log();
        console.log();
        console.log();
        console.log();
        ...
        const event2 = await agent.post('/api/v1/events/add')
            ...
            .set('Accept', 'application/json')
            .expect('Content-Type', /json/);
        console.log(JSON.stringify(event2.body));
        assert.nestedPropertyVal(event2, 'body.code', 1504);
        assert.nestedPropertyVal(event2, 'body.message', 'Date is crossed with other event');
    });
});
